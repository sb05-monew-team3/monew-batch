...
@Component
@Slf4j
public class NotificationProcessor implements ItemProcessor<Interest, Interest> {
	...
	public void beforeStep(StepExecution stepExecution) {
		...
		log.info("[ 기사 알림 추가] Processor 초기화 완료");
	}

	@Override
	public Interest process(Interest input) {
		log.info("[ 기사 알림 추가] 관심사 처리 시작 - ID: {}", input.getId());
		...
		for (InterestKeyword interestKeyword : interest.getInterestKeywords()) {
			...
			log.info("[ 기사 알림 추가] 키워드 '{}' - 추가된 기사: {}건", k, count);
		}

		log.info("[ 기사 알림 추가] 관심사 '{}' - 총 {}건의 새 기사", interest.getName(), sumOfAddedCount);
